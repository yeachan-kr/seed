...
class NTXentLoss(_Loss):
    def __init__(self, args, batch_size, temperature=1.0, use_cosine_similarity=True, device="cuda"):
    ...
    def forward(self, zis, zjs):
        """
        :param zis: (B, D)      # B: batch size, D: dimension of the representation ~~ sentence embedding of the original tokenized sentence
        :param zjs: (B, D)      # B: batch size, D: dimension of the representation ~~ sentence embedding of the augmented tokenized sentence
        :return:
        """
        ...
        l_pos = torch.diag(similarity_matrix, self.batch_size)          # (B, )
        r_pos = torch.diag(similarity_matrix, -self.batch_size)         # (B, )

        positives = torch.cat([l_pos, r_pos]).view(2 * self.batch_size, 1)     # (2B, 1)
        sample_mask = self.mask_samples_from_same_repr[:(self.batch_size * 2), :(self.batch_size * 2)]
        negatives = similarity_matrix[sample_mask].view(2 * self.batch_size, -1)
...
class AlignLoss(_Loss):
    ...
    def forward(self, zis, zjs):
        ...
        return torch.mean(1 - a) * self.temperature